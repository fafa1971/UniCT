...
/* INIZIALIZZA LA MODALITA' GRAFICA VGA (640x480x16). */
{
...
/* DISEGNA UNA STELLA CON CENTRO (x,y) E RAGGIO rmax. */
{
  int rmin,punti[20];
...
void Intro(void)
/* VISUALIZZA SULLO SCHERMO L'INTRODUZIONE ANIMATA. */
{
...
void Menu(void)
/* IL MENU PRINCIPALE PROVVEDE A SELEZIONARE L'OPZIONE DESIDERATA. */
{
  int i;
...
/* INTRODUZIONE - PAG.1/6 */
{
  ...
  Riga[11]="nodo la presenza di 4 code a priorit… differente,";
...
/* INTRODUZIONE - PAG.5/6 */
{
  ...
  Riga[13]="   In ogni caso Š sempre possibile tornare alla tabella";
  Riga[14]="coi valori numerici prima di ritornare al menu principale.";
}

void Pagina_6(void)
/* INTRODUZIONE - Pag.6/6 */
{
...
/* PROVVEDE A STAMPARE SULLO SCHERMO LA PAGINA DI INTRODUZIONE DESIDERATA. */
{
...
/* SFOGLIA L'INTRODUZIONE AVANTI E INDIETRO. */
{
  unsigned Pagina=1;
...
/* STAMPA NELLA PAGINA "DATA INPUT" IL VALORE ATTUALE DEI PARAMETRI
   DELLA RETE. */
{
  char Stampa[20];
  unsigned IndPrior;
  ...
  if(Evidenziato==5) setcolor(WHITE); else setcolor(BLUE);
  moveto(40,410); outtext("Tempo di simulazione = ");
  gcvt(T_Simul/1000,5,Stampa); outtext(Stampa); outtext(" sec");
...
/* PERMETTE L'INPUT DA TASTIERA DEI PARAMETRI DELLA RETE. */
{
  int Parametro=0;
  ...
  do
  {
    ...
    if(Tasto==13)
    {
      ...
      switch(Parametro)
      {
	case 0 : printf("Numero di Nodi (max 180) : ");
		 Nodi=atoi(gets(Buffer));
		 if(Nodi==0) Nodi=30;
		 if(Nodi<2) Nodi=2;
		 if(Nodi>180) Nodi=180; break;
	...
	case 5 : printf("Tempo da simulare [sec] (consigliato %g) : ",
			(float)(Nodi*Lunghezza)/(10000*Data_Rate));
		 T_Simul=1000*atof(gets(Buffer));
		 if(T_Simul==0) T_Simul=(float)(Nodi*Lunghezza)/(10*Data_Rate);
		 if(T_Simul<500) T_Simul=500;
		 if(T_Simul>1e6) T_Simul=1e6; break;
      }
      Beep();
      Stampa_Dati_Attuali(Parametro);
    }
